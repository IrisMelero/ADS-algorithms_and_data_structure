...
public class ABB {
    ...
    // a busca pelo próximo elemento pode ser muito complicada, pois aas ramificações podem acontecer de 
    // várias maneiras; uma possibilidade é percorrer a árvore "emOrdem" e buscar o elemento na lista
    // o curso é O(n)
    ...
    public int proximo (int x) {
        if (estaVazia()) return -1; // decisão de projeto
        ArrayList<Integer> lista = new ArrayList<>();
        constroiLista (lista, raiz);
        System.out.println(lista);
        if (!lista.contains(x)) return -1; // decisão de projeto
